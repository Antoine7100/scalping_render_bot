...
async def launch_telegram_bot():
    # Supprimer tout ancien webhook actif pour éviter les conflits
    await ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).build().bot.delete_webhook(drop_pending_updates=True)
